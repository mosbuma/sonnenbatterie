...
const simulateCharging = () => {
  ...
  const chargeRate =
    batteryState.BatteryCharging && batteryState.OperatingMode === "1"
      ? batteryState.chargeSetpoint
      : 0;
  const dischargeRate =
    !batteryState.BatteryCharging && batteryState.OperatingMode === "1"
      ? batteryState.dischargeSetpoint
      : 0;

  // Update current power based on charge/discharge rates
  if (chargeRate > 0) {
    batteryState.currentPowerWh = Math.min(
      batteryState.currentPowerWh + chargeRate * (elapsedTime / 3600),
      batteryState.maxPowerWh
    );
  } else if (dischargeRate > 0) {
    batteryState.currentPowerWh = Math.max(
      batteryState.currentPowerWh - dischargeRate * (elapsedTime / 3600),
      0
    );
  }
  batteryState.charging =
    batteryState.charging && batteryState.currentPowerWh > 0;
  batteryState.discharging =
    batteryState.discharging && batteryState.currentPowerWh < 0;
};
simulateCharging();
...
app.listen(PORT, () => {
  console.log(`battery-sim server running on http://localhost:${PORT}`);
});
