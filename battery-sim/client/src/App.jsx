...
const BATTERY_API = import.meta.env.VITE_BATTERY_API_URL || 'http://localhost:5001';
const AUTOPILOT_API = import.meta.env.VITE_AUTOPILOT_API_URL || 'http://localhost:5002';
...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchBatteryState = async () => {
      try {
        const batteryResponse = await fetch(`${BATTERY_API}/api/v2/status`).catch(() => ({ ok: false }));
    ...
    const fetchTimeState = async () => {
      try {
        const timeResponse = await fetch(`${BATTERY_API}/api/v2/time/status`).catch(() => ({ ok: false }));
  ...
  useEffect(() => {
    const fetchAutopilotState = async () => {
      try {
        const autopilotResponse = await fetch(`${AUTOPILOT_API}/api/autopilot/v1/status`).catch(() => ({ ok: false }));
        setApiStatus(prevStatus => ({
          ...prevStatus,
  ...
  const handleToggleAutopilot = async () => {
    await fetch(`${AUTOPILOT_API}/api/autopilot/v1/enable/${!autopilotState.enabled}`, {
      method: "POST",
    });
  };
  ...
  const handleStartTime = async () => {
    try {
      const response = await fetch(`${BATTERY_API}/api/v2/time/start`, { method: 'POST' });
      const result = await response.json();
  ...
  const handleStopTime = async () => {
    try {
      const response = await fetch(`${BATTERY_API}/api/v2/time/stop`, { method: 'POST' });
      const result = await response.json();
      setTimeRunningState(result.running);
    } catch (error) {
  ...
  const handleCharge = async () => {
    await fetch(`${BATTERY_API}/api/v2/setpoint/charge/2000`, { method: "POST" });
  };

  const handleDischarge = async () => {
    await fetch(`${BATTERY_API}/api/v2/setpoint/discharge/2000`, { method: "POST" });
  };

  const handleStop = async () => {
    await fetch(`${BATTERY_API}/api/v2/setpoint/discharge/0`, { method: "POST" });
  ...
  return (
    ...
    <div className="container mx-auto max-w-4xl">
      <h1 className="text-3xl font-bold mb-8 text-center">Battery Simulator</h1>
      
      <div className="flex justify-center">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white p-6 rounded-lg shadow-md relative">
            <StatusLed status={apiStatus.battery} />
            <h2 className="text-xl font-semibold mb-4 text-center">Battery Status</h2>
            <div className="space-y-2">
              <p className="text-gray-600">Timestamp: {currentTime}</p>
              <p className="text-gray-600">Operating Mode: {batteryState.OperatingMode}</p>
              <p className="text-gray-600">
                Battery Charging: {batteryState.BatteryCharging ? "Yes" : "No"}
              </p>
              <p className="text-gray-600">State of Charge: {batteryState.USOC}%</p>
              <p className="text-gray-600">Minimum Charge Level: {minChargeLevel}%</p>
              <p className="text-gray-600">Production: {batteryState.Production_W} W</p>
              <p className="text-gray-600">Charging Power: {batteryState.ChargingPower_W} W</p>
              <p className="text-gray-600">Discharging Power: {batteryState.DischargingPower_W} W</p>
            </div>
          </div>

          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold mb-4 text-center">Simulation Control</h2>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8 relative">
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            <span className="text-gray-700">AUTOPILOT</span>
